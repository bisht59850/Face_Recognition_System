...
class Face_Recognition:
    def __init__(self,root):
        ...
        # 1st image
        img_top=Image.open(r"E:\Python_projects\facial_recognition\images\3f.jpg")
        ...
        # 2nd image 
        img_bottom=Image.open(r"E:\Python_projects\facial_recognition\images\fdetection.jpg")
    ...
    def face_recog(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for(x,y,w,h) in features:
                ...
                # print(f"Fetched Name: {n}")

                # n="+".join(n)

                if n is not None and len(n) > 0:
                    n = n[0]  # Extract the value from the tuple
                else:
                    n = "Unknown"
                ...
                if r is not None and len(r) > 0:
                    r = r[0]  # Extract the value from the tuple
                else:
                    r = "Unknown"
                ...
                if d is not None and len(d) > 0:
                    d = d[0]  # Extract the value from the tuple
                else:
                    d = "Unknown"
                ...
                if i is not None and len(i) > 0:
                    i = i[0]  # Extract the value from the tuple
                else:
                    i = "Unknown"    
                ...
                if confidence>77:
                    cv2.putText(img,f"ID:{i}",(x,y-80),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,255,0),3)
